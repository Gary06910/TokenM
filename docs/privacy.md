# To Know Privacy Policy

To Know is local-first, but it is not a pure offline application. It processes usage logs and usage statistics locally and does not send analytics or telemetry to the project maintainer. To Know has no default public usage-collection server; network access occurs only for documented or user-enabled features.

## Network features

To Know may make network requests for these documented or user-enabled features:

- Packaged builds check GitHub Releases for update metadata and downloads. GitHub is the update service for the To Know Desktop feed; see the [GitHub Privacy Statement](https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement).
...
- To Know Android notifications send an allowlisted completion event from Desktop to the user-configured To Know backend / uniCloud only after the user pairs the Desktop and enables notifications. The configured backend then handles task persistence and Android notification delivery through the To Know mobile path.

Review the privacy policy of the service receiving data before enabling a provider-backed integration or connecting a self-hosted deployment. To Know does not sell usage data to advertisers.

## To Know Android notifications
...
The current To Know Android system notification uses generic completion text and task identity. It does not include prompt text, reply text, `cwd`, terminal output, source code, file contents, credential material, token material, task body, or summary content. Notification delivery can be disabled by the user and may fail; a provider success response is required before the backend records it as sent.

## Multi-device sync

Multi-device sync is optional and has no To Know-operated default server. The operator chooses and controls the destination hub, whether it runs in the app, on a self-hosted Node server, or on Cloudflare Workers.
...
Data retention and access on a synchronized or notification-enabled deployment are controlled by the operator of that backend and its infrastructure provider. To Know does not sell this data to advertisers.
