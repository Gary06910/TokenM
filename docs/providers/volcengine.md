...
Discovery is on by default when the Volcengine provider is enabled; no extra arkcli opt-in is required. To Know checks the inherited PATH first, then common npm, Homebrew, Bun, and local-bin locations. Set `TOKEN_MONITOR_VOLCENGINE_ARKCLI=0` to disable discovery, or `TOKEN_MONITOR_ARKCLI_COMMAND` to an explicit executable path for another installation.
