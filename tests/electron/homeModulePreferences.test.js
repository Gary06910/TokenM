...
const modules = [
  ...
  { id: 'cacheHit', label: 'Cache Hit Rate', insertBefore: 'trends' },
  { id: 'trends', label: 'Activity' }
];

test('defaultHomeModulePreferences keeps the full Home module order and hides only the existing secondary modules', () => {
  assert.equal(DEFAULT_HOME_MODULE_ORDER, 'limits,tool,device,model,cacheHit,trends');
  assert.deepEqual(defaultHomeModulePreferences(), {
    homeModuleOrder: 'limits,tool,device,model,cacheHit,trends',
...
test('default Home module preferences show the original overview modules first', () => {
  ...
  assert.deepEqual(
    ...
    ['limits', 'model', 'cacheHit', 'trends']
  );
});

test('normalizeHomeModuleOrder drops invalid ids and inserts new modules at their declared boundary', () => {
  assert.deepEqual(
    normalizeHomeModuleOrder('device,unknown,device,limits', modules),
    ['device', 'limits', 'tool', 'model', 'cacheHit', 'trends']
  );
});
...
test('normalizeHiddenHomeModules keeps known ids but never hides every Home module', () => {
  ...
  assert.equal(normalizeHiddenHomeModules('limits,tool,device,model,trends', modules), 'limits,tool,device,model,trends');
});
...
test('moveHomeModuleOrder and reorderHomeModuleOrder update saved order', () => {
  assert.equal(
    ...
    'limits,device,tool,model,cacheHit,trends'
  );
  assert.equal(
    reorderHomeModuleOrder('limits,tool,device,model,trends', modules, 'trends', 1),
    'limits,trends,tool,device,model,cacheHit'
  );
});
