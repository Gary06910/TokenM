...
test('the scan command aligns Tokscale HOME with the collector home', () => {
  const homeDir = 'C:\\Users\\me';
  const command = tokscaleCommand({ homeDir, platform: 'win32' });
  assert.equal(command.env.HOME, homeDir);
});
