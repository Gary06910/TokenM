...
function callbackPage(ok) {
  ...
  const detail = ok
    ? 'Return to Token M to finish connecting this account.'
    : 'Return to Token M for details.';
...
async function runAntigravityOAuthLogin(options = {}) {
  ...
  if (!client) {
    throw loginError(
      ...
      'Antigravity OAuth is unavailable in this build. Update Token M or configure ANTIGRAVITY_OAUTH_CLIENT_ID and ANTIGRAVITY_OAUTH_CLIENT_SECRET.'
    );
  }
